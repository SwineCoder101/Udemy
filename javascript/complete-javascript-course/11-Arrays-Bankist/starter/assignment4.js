...
dogs.forEach(dog => {
  ...
  dog.status = (function () {
    ...
    } else if (dog.curFood < dog.recFood * 0.9) return 'ownersEatTooLittle';
    else {
      return 'ownersEatOk';
    }
  })();
...
// 2. Find Sarah's dog and log to the console whether it's eating too much or too little. HINT: Some dogs have multiple owners, so you first need to find Sarah in the owners array,
// and so this one is a bit tricky (on purpose) 🤓
const sarahsDogs = dogs
  .filter(dog => dog.owners.includes('Sarah'))
  .map(dog =>
    console.log(
      `curFood: ${dog.curFood} recommended food: ${dog.recFood}, dog owners are: ${dog.owners}, ${dog.status}`
    )
  );
