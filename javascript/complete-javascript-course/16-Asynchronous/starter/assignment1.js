...
with the request. Remember, fetch() does NOT reject the promise in this case. So create an error to reject the promise yourself, with a 
...
const whereAmI = function (lat, lng) {
  fetch(
    `https://geocode.xyz/${lat},${lng}?geoit=json&auth=392647314157288403645x75208`
  )
    .then(response => {
    ...
    .then(data => {
      if (!data.country) throw new error(`unable to find country from geocode`);
      console.log(`you are in ${data.city}, ${data.country}`);
      return fetch(URL + data.country);
    })
    .then(response => response.json())
    .then(data => renderCountry(data[0]))
    .catch(error => console.log('Unable to get specified country \n', error));
};

whereAmI(51.50354, -0.12768);
whereAmI(501.50354, -0.12768);
